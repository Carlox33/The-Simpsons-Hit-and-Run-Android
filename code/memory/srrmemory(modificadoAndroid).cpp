...
static long gettid_() { return (long)syscall(SYS_gettid); }
#endif
...
void HeapStack::Push (GameMemoryAllocator alloc)
{
	...
	#ifndef FINAL
#ifdef RAD_ANDROID
  __android_log_print(ANDROID_LOG_INFO, "SRR2_HEAP",
    "PUSH  tid=%d this=%p curPos=%d alloc=%d stackPtr=%d pushNo=%d",
    gettid(),
    this,
    m_CurPos,
    (int)alloc,
...
#ifndef FINAL
#ifdef RAD_ANDROID
    __android_log_print(ANDROID_LOG_ERROR, "SRR2_HEAP",
        "POP ENTER: tid=%d this=%p curPos=%d stackPtr=%d alloc=%d",
        gettid(), this, m_CurPos, currentStackPointer, (int)alloc);
#endif
#endif

    rAssertMsg(currentStackPointer > 0, "HeapStack: Pop(alloc) with empty debug stack (underflow)");
    --currentStackPointer;
    pushNumberStack[currentStackPointer] = -1;

    // ... (tu código de mismatch igual que lo tienes)
...
HeapManager* HeapManager::GetInstance ()
{
...
#ifdef RAD_ANDROID
        __android_log_print(ANDROID_LOG_INFO, "SRR2_HEAP",
            "TLS CREATE: tid=%d s_Instance(before)=%p", tid, (void*)s_Instance);
#endif

        ::radThreadCreateLocalStorage (&s_Instance, GMA_PERSISTENT);
        s_Instance->SetValue (0);

#ifdef RAD_ANDROID
        __android_log_print(ANDROID_LOG_INFO, "SRR2_HEAP",
            "TLS CREATED: tid=%d s_Instance(after)=%p", tid, (void*)s_Instance);
#endif
    }
#ifdef RAD_ANDROID
    else
    {
        __android_log_print(ANDROID_LOG_INFO, "SRR2_HEAP",
            "TLS EXISTS: tid=%d s_Instance=%p", tid, (void*)s_Instance);
    }
#endif
...
#ifdef RAD_ANDROID
    __android_log_print(ANDROID_LOG_INFO, "SRR2_HEAP",
        "TLS GETVALUE: tid=%d s_Instance=%p p=%p", tid, (void*)s_Instance, p);
#endif
...
#ifdef RAD_ANDROID
        __android_log_print(ANDROID_LOG_INFO, "SRR2_HEAP",
            "HM CREATE ENTER: tid=%d currentHeap=%d", tid, (int)currentHeap);
#endif
