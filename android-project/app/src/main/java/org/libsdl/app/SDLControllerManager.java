...
import static org.libsdl.app.SDLAudioManager.TAG;
import static org.libsdl.app.SDLHapticHandler.is32Bit;
...
public class SDLControllerManager
{
    ...
    public static boolean rumbleFallback(float intensity, int length) {
        if (is32Bit()) {
            return false; // 🚫 Bloqueamos vibración en 32 bits
        }
        ...
        mHapticHandler.pollHapticDevices();
        return mHapticHandler.runFallback(intensity, length);
    }

    public static void stopRumbleFallback() {
        if (is32Bit()) {
            return;
        }
